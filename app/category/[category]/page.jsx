...
const Page = () => {
    ...
    // Fetch categories + blogs
     useEffect(() => {
        axios.get("https://dameta1.com/dameta-backend/public/api/all/category")
            .then(res => setFetchedCategories(res.data.categories))
    ...
    const filteredBlogs = decoratedBlogs && decoratedBlogs.filter(item =>
        item.category.some(
  ...
  return new Date(dateString).toLocaleDateString('en-US', {
    ...
    console.log('decoratedBlogs', decoratedBlogs)
                            ...
                            <div className="absolute bottom-0 flex gap-1 z-10 left-0">
                                <div className="bg-white rounded-full p-2 px-3 sm:p-3 sm:px-5">
                                    <p className="exo text-xs sm:text-sm text-zinc-400">
                                        {formatDate(catog.date)} / {catog.owner}
                                    </p>
                                </div>
                            </div>
                        </div>
